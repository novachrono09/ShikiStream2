...
const VideoPlayerSection = () => {
  ...
  const { data: episodeData, isLoading: isEpisodeDataLoading, refetch } = useGetEpisodeData(
    selectedEpisode,
    ...
    const checkServer = async (server: any, type: 'sub' | 'dub') => {
      try {
        const { data: newData, isSuccess } = await refetch({
          queryKey: [GET_EPISODE_DATA, selectedEpisode, server.serverName, type],
        });
  ...
  async function onHandleAutoSkipChange(value: boolean) {
    ...
    const res = await pb.collection('users').update(auth.id, { autoSkip: value });
    if (res) {
  ...
  useEffect(() => {
    ...
    if (!existingAnime) {
      ...
      const episodeAlreadyWatched = existingAnime.episodes.includes(selectedEpisode);
      ...
      <>
        ...
        <div className="mt-4">
          <Alert variant="destructive" className="text-red-400">
            ...
            <AlertDescription>
              The original video source for this episode is currently unavailable. A fallback player has been
              provided for your convenience. We recommend using an ad blocker for a smoother viewing experience.
            </AlertDescription>
          </Alert>
        </div>
      </>
    ...
    <div>
      ...
      <div className="flex flex-row bg-[#0f172a]  items-start justify-between w-full p-5">
        <div>
          {availableServers.filter((s) => s.serverType === 'sub').length > 0 && (
            <div className="flex flex-row items-center space-x-5">
              ...
              {availableServers
                .filter((s) => s.serverType === 'sub')
                .map((s, i) => (
                  <Button
                    size="sm"
                    key={i}
                    className={`uppercase font-bold ${
                      activeServer.serverName === s.serverName &&
                      activeServer.serverType === 'sub' &&
                      'bg-red-300'
                    }`}
        ...
        <div className="flex flex-row items-center space-x-2 text-sm">
          <Switch checked={autoSkip} onCheckedChange={(e) => onHandleAutoSkipChange(e)} id="auto-skip" />
          <p>Auto Skip</p>
        </div>
      </div>
    </div>
